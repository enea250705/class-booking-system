...
export default async function DirectBookingsPage({ searchParams }: { searchParams: { classId?: string } }) {
  ...
  // Get the class ID from query params
    ...
    return (
      <div className="min-h-screen bg-black p-4 flex flex-col items-center justify-center">
        <div className="max-w-md w-full bg-gray-900 p-6 rounded-lg border border-gray-800">
          <h1 className="text-xl font-bold text-white mb-2">No Class ID Provided</h1>
          <p className="text-gray-400 mb-6">
            Please provide a classId query parameter
          </p>
  ...
  try {
    ...
    // Get class bookings from the new direct API endpoint
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || ''}/api/admin/classes/direct-bookings?classId=${classId}`;
